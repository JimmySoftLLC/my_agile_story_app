...
import 'package:my_agile_story_flutter_app/view/new_bug.dart';
import 'package:my_agile_story_flutter_app/view/new_user_story.dart';
import 'package:my_agile_story_flutter_app/view/edit_bug.dart';
...
import 'package:my_agile_story_flutter_app/view/bug_cards.dart';
import 'dart:async';
...
class _MyLoggedInPageState extends State<MyLoggedInPage> with WidgetsBindingObserver{
  ...
  List<Widget> bugCardsToDo = <BugCard>[];
  List<Widget> bugCardsDoing = <BugCard>[];
  List<Widget> bugCardsVerify = <BugCard>[];
  List<Widget> bugCardsDone = <BugCard>[];
  ...
  void editBugInContext() {
    updateScrollPositions();
    Navigator.pushReplacementNamed(context, EditBug.id);
  }

  void moveBugToNextPhaseInContext() {
    updateScrollPositions();
    messagePopupNoDismiss('',Colors.black,'Moving user story to next phase please wait',context);
    updateBug(myProjects[myLastSelectedProject],
        myBugs[myLastSelectedBug], context);
  }

  void changeBugPriorityUsingSliderInContext() {
    updateScrollPositions();
  ...
  void moveUserStoryToNextPhaseInContext() {
    updateScrollPositions();
    messagePopupNoDismiss('',Colors.black,'Moving user story to next phase please wait',context);
    updateUserStory(myProjects[myLastSelectedProject],
  ...
  void initState() {
    ...
    if (myLastSelectedProject >= 0) {
      ...
      bugCardsToDo = <BugCard>[];
      bugCardsDoing = <BugCard>[];
      bugCardsVerify = <BugCard>[];
      bugCardsDone = <BugCard>[];
      ...
      bugCardsToDo = updateBugCards('0');
      bugCardsDoing = updateBugCards('1');
      bugCardsVerify = updateBugCards('2');
      bugCardsDone = updateBugCards('3');
              ...
              children: <Widget>[
                ...
                IconButton(
                  ...
                  onPressed: () {
                    if (myLastSelectedProject != -1) {
                      Navigator.pushNamedAndRemoveUntil(context,
                          NewBug.id, (Route<dynamic> route) => false);
                    } else {
                      messagePopup('Note',Colors.black,
